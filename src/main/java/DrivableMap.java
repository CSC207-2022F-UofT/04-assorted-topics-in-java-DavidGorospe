...
class DrivableMap {
    ...
    {
        Drivable[] speeds = drivable_map.values().toArray(new Drivable[drivable_map.size()]);
        for (int i = 0; i < drivable_map.size(); i++)
        {
            if (speeds[i].getMaxSpeed() == speed)
            {
                return true;
            }
        }
    ...
    public List<Tradable> getTradable()
    {
        ...
        Drivable[] items = drivable_map.values().toArray(new Drivable[drivable_map.size()]);
        for (int i = 0; i < drivable_map.size(); i++)
        {
            if (items[i] instanceof Tradable)
            {
                tradables.add((Tradable) items[i]);
            }
        }
        return tradables;
    }
